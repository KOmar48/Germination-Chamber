...
float tempValue = 68;
float tempHighValue = 69;
float tempLowValue = 67;
...
void update_LCD(LiquidCrystal_I2C lcd, float v1, float v2)
{
  ...
  else if (currentMode == 2)
  {
    ...
    lcd.setCursor(0,1);
    dtostrf(distanceCm,4,2,float_str1);
    sprintf(display_string, "%s cm", float_str1);
    lcd.print(display_string);
  }
...
void updateTemp()
{
  tempLowValue = tempValue - 1.0;
  tempHighValue = tempValue + 1.0;
}

void updateMoist()
{
  soilMoistLow = soilMoistValue - 1.0;
  soilMoistHigh = soilMoistValue + 1.0;
}
